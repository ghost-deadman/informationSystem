...
public interface ProjectBudgetMapper extends BaseMapper<ProjectBudget> {
     ...
     * @return 分页数据
     */
    @Select("SELECT pb.project_budget_id, p.project_name, su.user_name, pb.money, p.unit_id, un.name, pb.opinion_id, pb.date, pb.describes " +
    ...
    @Select("SELECT pb.project_budget_id, p.project_name, su.user_name, pb.money, p.unit_id, un.name,pb.opinion_id, pb.date, pb.describes " +
            ...
            "WHERE p.project_budget_id = pb.project_budget_id " +
            "AND un.unit_id = p.unit_id AND p.project_user_id = su.id " +
            "AND pb.deleted = '0' AND un.unit_id = #{unitId} AND pb.status = #{status}")
    ...
    @Select("SELECT pb.project_budget_id, p.project_name, su.user_name, pb.money, p.unit_id, un.name,pb.opinion_id, pb.date, pb.describes " +
            ...
            "WHERE p.project_budget_id = pb.project_budget_id " +
            "AND un.unit_id = p.unit_id AND p.project_user_id = su.id " +
            "AND pb.deleted = '0' AND pb.status = #{status}")
